...
import kwh.PublicCookedFood.board.service.command.BoardSectionCreateCommand;
import kwh.PublicCookedFood.board.service.command.BoardSectionReorderCommand;
import kwh.PublicCookedFood.board.service.command.BoardSectionUpdateCommand;
...
public class AdminBoardController {
    ...
    public ResponseEntity<List<BoardSectionResponse>> getSections() {
        return ResponseEntity.ok(boardSectionQueryService.getAllSections().stream()
    ...
    public ResponseEntity<BoardSectionResponse> createSection(@Valid @RequestBody BoardSectionCreateRequest request) {
        BoardSectionResponse response = BoardSectionResponse.from(
                boardSectionCommandService.createSection(
                        BoardSectionCreateCommand.of(
    ...
    public ResponseEntity<List<BoardSectionResponse>> reorderSections(
        ...
        return ResponseEntity.ok(
                boardSectionCommandService.reorderSections(BoardSectionReorderCommand.of(request.getSectionIds())).stream()
                        .map(BoardSectionResponse::from)
                        .toList()
        );
    }
    ...
    public ResponseEntity<BoardSectionResponse> updateSection(
        ...
        BoardSectionResponse response = BoardSectionResponse.from(
                boardSectionCommandService.updateSection(
                        BoardSectionUpdateCommand.of(
                                sectionId,
                                request.getSectionName(),
                                request.getDisplayOrder(),
                                request.getActive()
    ...
    public ResponseEntity<Void> deleteSection(
        ...
        boardSectionCommandService.deleteSection(sectionId);
    ...
    public ResponseEntity<BoardPolicyResponse> updatePolicy(
        ...
        boardAdminPolicyFacade.updateBoardPolicy(
                BoardPolicyUpdateCommand.fullUpdate(
                        request.getFeaturedLikeThreshold(),
                        request.getThumbnailDisplayMode(),
                        actorAccountId
                )
        );
        return ResponseEntity.ok(loadPolicy());
    }
    ...
    public ResponseEntity<BoardPolicyResponse> updateFeaturedThreshold(
        ...
        Long actorAccountId = resolveActorAccountId(account);
        boardAdminPolicyFacade.updateBoardPolicy(
                BoardPolicyUpdateCommand.featuredThreshold(request.getFeaturedLikeThreshold(), actorAccountId)
        );
        return ResponseEntity.ok(loadPolicy());
    }
    ...
    public ResponseEntity<BoardPolicyResponse> updateThumbnailDisplayMode(
        ...
        Long actorAccountId = resolveActorAccountId(account);
        boardAdminPolicyFacade.updateBoardPolicy(
                BoardPolicyUpdateCommand.thumbnailDisplayMode(request.getThumbnailDisplayMode(), actorAccountId)
        );
        return ResponseEntity.ok(loadPolicy());
    }
    ...
    private BoardPolicyResponse loadPolicy() {
        return boardAdminPolicyFacade.loadPolicyData().policy();
    }
}
