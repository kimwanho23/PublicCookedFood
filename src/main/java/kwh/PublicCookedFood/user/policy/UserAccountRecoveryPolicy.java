...
@Component
public class UserAccountRecoveryPolicy {
    ...
    public String validateFindEmailRequest(String name, String phoneNumber) {
        if (isBlank(name) || isBlank(phoneNumber)) {
            return "이름과 전화번호를 모두 입력해주세요.";
        }
        return null;
    }

    public String validateResetCodeRequest(String email, String name, String phoneNumber) {
        if (isBlank(email) || isBlank(name) || isBlank(phoneNumber)) {
            return "이메일, 이름, 전화번호를 모두 입력해주세요.";
        }
