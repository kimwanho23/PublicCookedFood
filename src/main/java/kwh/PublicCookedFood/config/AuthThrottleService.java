...
public class AuthThrottleService {
    ...
    private String loginKey(HttpServletRequest request, String email) {
        return "login:" + clientIpResolver.resolve(request) + ":" + normalizeText(email);
    }

    private String recoveryKey(HttpServletRequest request, String action, String identity) {
        return "recovery:" + normalizeText(action) + ":" + clientIpResolver.resolve(request) + ":" + normalizeText(identity);
    }
    ...
    private void cleanupIfNeeded() {
        ...
        long now = System.currentTimeMillis();
        long maxWindowMillis = Math.max(LOGIN_WINDOW_MILLIS, RECOVERY_WINDOW_MILLIS);
